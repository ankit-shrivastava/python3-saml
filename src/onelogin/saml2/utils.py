...
class OneLogin_Saml2_Utils(object):
    ...
    TIME_FORMAT_WITH_FRAGMENT_2 = re.compile(r'^(\d{4,4}-\d{2,2}-\d{2,2}T\d{2,2}:\d{2,2}:\d{2,2})(\.\d*)(\+\d*):\d{2,2}?Z?$')
    ...
    def parse_SAML_to_time(timestr):
        ...
        try:
            ...
            try:
                ...
                if not elem:
                    elem = OneLogin_Saml2_Utils.TIME_FORMAT_WITH_FRAGMENT_2.match(timestr)
                    if not elem:
